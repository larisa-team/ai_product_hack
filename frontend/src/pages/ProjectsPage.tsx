...
export default function ProjectsPage() {
  ...
  const projects = useQuery({
    queryKey: ["projects"],
    queryFn: async () => (await projectClient.listProjects({})).projects,
  });
  ...
  const create = useMutation({
    mutationFn: async () => {
      // Типы полей приходят из proto — лишнее поле не соберётся
      const res = await projectClient.createProject({
        name: name.trim(),
        topic: topic.trim(),
        filters: lines(filters).map((prompt) => ({ prompt })),
        sources: lines(sources).map((telegram) => ({
  ...
  return (
    <Stack gap="xl">
      <Stack gap="xs">
        ...
        {projects.data?.length === 0 && <Text c="dimmed">Пока нет проектов.</Text>}
        {projects.data?.map((p) => (
